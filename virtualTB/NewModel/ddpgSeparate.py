...
class Actor(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.mu = nn.Linear(cfg.out_dim+3, num_outputs)
    ...
    def forward(self, inputs):
        ...
        y = x[:, 88:]
        y = y.reshape([-1, 3])
        x = self.linear1(x[:, :88].reshape([-1, 88]))
        x = F.relu(x)
        x = self.linear2(x)
        x = F.relu(x)

        max_cols = max(x.size(-1), y.size(-1))
        tensor1_padded = torch.nn.functional.pad(x, (0, max_cols - x.size(-1)))
        tensor2_padded = torch.nn.functional.pad(y, (0, max_cols - y.size(-1)))
        new_feature = torch.cat((tensor1_padded, tensor2_padded), dim=-1)[:, :cfg.out_dim+3].reshape([-1, cfg.out_dim+3])
        mu = torch.tanh(self.mu(new_feature))  # (-1, 1)
...
if __name__ == "__main__":
    ...
    wandb.init(name='DDPGNew_withoutAE', project="DRLRecVTB")
    cfg = get_args()
    ...
    for i_episode in tqdm(range(int(cfg.train_eps/5))):
        ...
        if i_episode % (int(cfg.test_freq/4)) == 0:
            ...
            print("Episode: {}, total numsteps: {}, average reward: {}, CTR: {}".format(
                i_episode, episode_step, episode_reward / len(test_data), episode_reward / episode_step / 10))  # 每页显示10个item
    ...
    test_result_df.to_csv("./new_result/ddpg_Separate.csv")
