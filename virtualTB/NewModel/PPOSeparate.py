...
class Actor(nn.Module):
    def __init__(self, n_states, n_actions, cfg):
        ...
        self.actor = nn.Sequential(
            nn.Linear(n_states-3, cfg.hidden_dim),
            nn.Tanh(),
        ...
        self.actor1 = nn.Sequential(nn.Linear(cfg.out_dim, n_actions),
                                    nn.Tanh())  # V(static)
        self.actor2 = nn.Sequential(nn.Linear(cfg.out_dim+3, n_actions),
                                    nn.Tanh())  # V(importance)
        self.mu = nn.Linear(n_actions, n_actions)
        self.sigma = nn.Linear(cfg.out_dim+3, n_actions)

    def forward(self, state):
        if state.dim() == 2:
            ae_state = self.actor(state[0][:88].view(1, -1))  # AE
            x1 = self.actor1(ae_state)
            y = state[0][88:].view(1, -1)
            max_cols = max(ae_state.size(1), y.size(1))
            tensor1_padded = torch.nn.functional.pad(ae_state, (0, max_cols - ae_state.size(1)))
            tensor2_padded = torch.nn.functional.pad(y, (0, max_cols - y.size(1)))

            new_feature = torch.cat((tensor1_padded, tensor2_padded), dim=1)[0][:cfg.out_dim + 3].view(1, -1)
            x2 = self.actor2(new_feature)
            mu = torch.add(x1, x2)/2  # [-2, 2]

            sigma = F.softplus(self.sigma(new_feature)) + 0.001

        else:  # state.dim() == 3, 训练的时候会遇到这种batch的情况
            ae_state = self.actor(state[:, :, :88])  # AE
            x1 = self.actor1(ae_state)
            y = state[:, :, 88:]
            max_cols = max(ae_state.size(2), y.size(2))
            tensor1_padded = torch.nn.functional.pad(ae_state, (0, max_cols - ae_state.size(2)))
            tensor2_padded = torch.nn.functional.pad(y, (0, max_cols - y.size(2)))

            new_feature = torch.cat((tensor1_padded, tensor2_padded), dim=2)[:, :, :cfg.out_dim + 3]
            x2 = self.actor2(new_feature)
            mu = torch.add(x1, x2) / 2  # [-2, 2]
...
if __name__ == "__main__":
    ...
    wandb.init(name='PPO_Separate', project="DRLRecVTB")

    cfg = get_args()
    cfg.env_name = "VirtualTB-v0"
    # cfg.device = "cuda"
    env, agent = env_agent_config(cfg, ae=False, seed=0)  # seed=0, 不设随机种子
    # env.to(cfg.device)
    test_result = []
    ...
    for i_ep in tqdm(range(cfg.train_eps)):
        ...
        while not done:
            ...
            ep_step += 1
            ep_reward += reward
            agent.memory.push(state, action.to(cfg.device), prob, val, reward, torch.squeeze(done).item())
        ...
        if (i_ep + 1) % cfg.test_freq == 0:
            ...
            for i in range(len(test_data)):
                ...
                while not done:
                    ...
                    ep_reward += reward
                    state = torch.tensor([state_])
            print(f"回合：{i_ep + 1}/{cfg.train_eps}，奖励：{ep_reward/len(test_data):.2f}，ctr：{ep_reward/ep_step/10:.4f}")
