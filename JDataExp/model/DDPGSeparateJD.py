...
class Actor(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.linear1 = nn.Linear(num_inputs-25, hidden_size)
        self.linear2 = nn.Linear(hidden_size, cfg.out_dim)
        self.linear3 = nn.Linear(25, cfg.dynamic_hidden)

        self.mu = nn.Linear(cfg.out_dim+cfg.dynamic_hidden, num_outputs)
    ...
    def forward(self, inputs):
        ...
        y = y.reshape([-1, 25])
        y = self.linear3(y)
        x = self.linear1(x[:, :16].reshape([-1, 16]))
        x = F.relu(x)
        x = self.linear2(x)
        x = F.relu(x)
        ...
        new_feature = torch.cat((
            tensor1_padded, tensor2_padded),
            dim=-1)[:, :cfg.out_dim+cfg.dynamic_hidden].reshape([-1, cfg.out_dim+cfg.dynamic_hidden])
